...
/* Global Variables ----------------------------------------------------------*/
GPSDATA_ST stgpsdata;             // 实时获取的gps数据
...
void NMEA_GPRMC_Analysis(nmea_msg *gpsx,u8 *buf)
{
    ...
    if(posx!=0XFF)
	{
		...
		gpsx->latitude=temp/NMEA_Pow(10,dx+2);	//得到°  ex:31
		rs=temp%NMEA_Pow(10,dx+2);				//得到'	  ex:5560926
		gpsx->latitude=(gpsx->latitude*NMEA_Pow(10,5)+(rs*NMEA_Pow(10,5-dx))/60)/100000.0;//转换为  .  ° 
	}	
...
void gps_init(void)
{
    ...
    Rate_chose(rate_5hz);               /* gps模块速率选择 */

    satellites_choose(Choose_mode);     /* gps模块单双频模式选择 */
		
    Ublox_Cfg_Prt(9600);				/* gps模块波特率设置 */

    MSG_choose();                       /* gps模块所需NMEA信息设置 */
